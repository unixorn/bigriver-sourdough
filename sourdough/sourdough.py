...
def readKnob(knobName, knobDirectory='/etc/knobs'):
    ...
    if not os.path.isfile(knobpath):
        print('readKnob: No such file: {}. This is normal, not all machines have every tag set.'.format(knobpath))
        return None
    if os.access(knobpath, os.R_OK):
        ...
        print('readKnob: Cannot read {}'.format(knobpath))
        return None
...
def loadVSphereSettings(knobName=DEFAULT_VSPHERE_KNOB, knobDirectory=DEFAULT_KNOB_DIRECTORY):
    ...
    if os.path.isfile(fpath):
        this.logger.debug('Reading cached vSphere data from {}'.format(fpath))
        try:
...
def enableChefRuns():
    ...
    if os.path.isfile(DISABLE_SOURDOUGH_F):
        print("Removing {}".format(DISABLE_SOURDOUGH_F))
...
def disableChefRuns():
    ...
    with open(DISABLE_SOURDOUGH_F, 'w') as disabler:
        print('Creating {}'.format(DISABLE_SOURDOUGH_F))
...
def enableDebugMode():
    ...
    with open(ENABLE_SOURDOUGH_DEBUGGING_F, 'w') as disabler:
        print('Creating {}'.format(ENABLE_SOURDOUGH_DEBUGGING_F))
        disabler.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def disableDebugMode():
    ...
    if os.path.isfile(ENABLE_SOURDOUGH_DEBUGGING_F):
        print('Removing {}'.format(ENABLE_SOURDOUGH_DEBUGGING_F))
